...
print(*list_1) # вывести в консоль снисок, без [] и запятых
...
# С помощью цикла for мы можем перебирать элементы списка

for item in list_1: # перебираем список по элементам
    print(item) # выводим при каждой итерации, элемент
...
# Чтобы узнать количество элементов в списке необходимо использовать функцию len()

len()
list_1 = [7, 9, 11, 13, 15, 17]
print(len(list_1)) # 6

# Мы обговорили с Вами создание списка и поняли, что мы можем пользоваться нумерацией,
# для того чтобы узнать какой элемент стоит на той или иной позиции. Но это не всегда
# удобно, особенно, когда список будет состоять из 1000, 1000000… элементов. В этом случае
